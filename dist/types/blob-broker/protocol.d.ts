...
import type { BlobPublication, RemoteDeleteAction } from "./publication.js";
import type { BlobBrokerSavingsStatus } from "./savings.js";
import type { DestinationRuntimeConfig } from "./uploader-runtime.js";
/** Hidden CLI selector used to re-enter the blob broker worker. */
export declare const BLOB_BROKER_WORKER_ARG = "__omp_worker_blob_broker";
/** Environment key carrying the control socket path the worker listens on. */
export declare const BLOB_BROKER_SOCKET_ENV = "OMP_BLOB_BROKER_SOCKET";
/** Environment key carrying the JSON {@link BlobBrokerWorkerConfig}. */
export declare const BLOB_BROKER_CONFIG_ENV = "OMP_BLOB_BROKER_CONFIG";
/** Stable broker daemon name for the shared blob broker. */
export declare const BLOB_BROKER_DAEMON_NAME = "omp.blob.broker";
/** Broker readiness regex matched against the banner printed by the worker. */
export declare const BLOB_BROKER_READY_PATTERN: string;
/** Banner printed on stdout once the exposure is up and control is listening. */
export declare function blobBrokerReadyBanner(baseUrl: string): string;
/** Resolve the control socket path for one project scope. */
export declare function blobBrokerEndpoint(runtimeDir: string): string;
...
export declare const BLOB_FETCH_EVENT_LIMIT = 100;
...
export declare const RENDER_CALLBACK_PATH = "/render/";
/** Header carrying {@link EnsureLazyRequest.callbackToken} on render callbacks. */
export declare const RENDER_CALLBACK_TOKEN_HEADER = "x-omp-blob-token";
